...
const char* BossRush_GetSettingName(uint8_t optionIndex, uint8_t language) {
    if (language >= LANGUAGE_MAX || BossRushOptions[optionIndex].name[language].empty()) {
...
const char* BossRush_GetSettingChoiceName(uint8_t optionIndex, uint8_t choiceIndex, uint8_t language) {
    if (language >= LANGUAGE_MAX || BossRushOptions[optionIndex].choices[choiceIndex][language].empty()) {
        language = LANGUAGE_ENG;
    }
